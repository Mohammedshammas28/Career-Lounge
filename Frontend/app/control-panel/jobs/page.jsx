...
export default function JobsAdminPage() {
        ...
        const sampleData = [
            {
                ...
                description: "We are seeking a Senior Software Engineer to lead the development of our cloud platforms.",
                requirements: [
                    "Bachelor's degree in Computer Science or related field",
                    "5+ years of experience with React, Node.js, and AWS",
            ...
            },
            {
                ...
                description: "Provide critical nursing care, manage patient rehabilitation programs, and lead hospital shifts.",
                requirements: [
                    "Bachelor of Science in Nursing (BSN)",
                    "DOH/HAAD License to practice in Abu Dhabi",
                    "3+ years of clinical nursing experience"
                ],
                responsibilities: [
                    "Deliver expert direct care to patients",
    ...
    const handleBulkSubmit = async (e) => {
        ...
        if (!bulkJsonText.trim()) {
            setBulkError("Please paste or upload a JSON array of job opportunities.");
            return;
        }
        ...
        if (!Array.isArray(parsedData)) {
            setBulkError("Invalid JSON format: The root element must be an Array `[...]` of job objects.");
            return;
        }

        if (parsedData.length === 0) {
            setBulkError("JSON array is empty. Please provide at least one job object.");
            return;
        }
            ...
            <div className="max-w-6xl mx-auto">
                ...
                <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4 mb-8 bg-white p-6 rounded-2xl border border-slate-100 shadow-[0_2px_8px_rgba(0,0,0,0.02)]">
                    ...
                    <div className="flex items-center gap-3">
                        {/* Bulk Add Button & Modal */}
                        <Dialog open={isBulkDialogOpen} onOpenChange={setIsBulkDialogOpen}>
                            <DialogTrigger asChild>
                                <Button
                                    variant="outline"
                                    className="border-slate-200 hover:border-slate-300 text-slate-700 bg-white shadow-sm font-semibold rounded-xl gap-2 h-11"
                                >
                                    <FileText className="w-5 h-5 text-emerald-600" /> Bulk Add (JSON)
                                </Button>
                            </DialogTrigger>
                            <DialogContent className="max-w-3xl w-[90vw] max-h-[85vh] flex flex-col p-0 overflow-hidden bg-white shadow-2xl rounded-2xl border-none">
                                <DialogHeader className="p-6 bg-slate-900 text-white relative">
                                    <DialogTitle className="text-xl font-bold tracking-wide flex items-center gap-2">
                                        ...
                                        Bulk Import Jobs (JSON)
                                    </DialogTitle>
                                    <p className="text-xs text-slate-300 mt-1">Upload a .json file or paste a JSON array of job opportunities below to insert them into the database.</p>
                                </DialogHeader>
                                ...
                                <div className="p-6 overflow-y-auto space-y-5">
                                    ...
                                    <div className="space-y-1.5">
                                        <label className="text-xs font-bold text-slate-600 uppercase tracking-wider flex justify-between">
                                            ...
                                            <span className="text-slate-400 normal-case font-normal">Format: Array of Job objects [...]</span>
                                        </label>
                                        <Textarea
                                            rows={12}
                                            value={bulkJsonText}
                                ...
                                <div className="p-4 border-t border-slate-100 bg-slate-50 flex items-center justify-between shrink-0">
                                    ...
                                    >
                                        {isSubmittingBulk ? (
                                            <>
                                                <Loader2 className="w-4 h-4 animate-spin" /> Inserting Jobs...
                                            </>
                                        ) : (
                                            <>
                                ...
                                <TableBody>
                                    {filteredJobs.map((job) => (
                                        <TableRow key={job._id} className="hover:bg-slate-50/50">
                                            ...
                                            <TableCell>
                                                <span className={
                                                    "inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[10px] font-black uppercase border " +
                                            ...
                                            <TableCell className="text-center">
                                                <button
                                                    ...
                                                    className={
                                                        job.isActive
                                                            ? "inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-extrabold transition-all border bg-emerald-50 text-emerald-700 border-emerald-200 hover:bg-emerald-100"
                                                            : "inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-extrabold transition-all border bg-rose-50 text-rose-700 border-rose-200 hover:bg-rose-100"
                                                    }
                                                >
                                                    {job.isActive ? (
                                                        <>
